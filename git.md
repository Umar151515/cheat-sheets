...
11. [Продвинутая работа с файлами](#продвинутая-работа-с-файлами)
12. [Теги (расширенный раздел)](#теги-расширенный-раздел)
13. [Git Worktrees](#git-worktrees)
14. [ПОЛНЫЙ .gitignore](#полный-gitignore)
15. [ПОЛНОЕ руководство по именованию](#полное-руководство-по-именованию)
...
`.gitignore` - это специальный файл в Git, который указывает, какие файлы и папки должны игнорироваться системой контроля версий. Файлы, соответствующие шаблонам в `.gitignore`, не будут отслеживаться Git, даже если они находятся в вашем рабочем каталоге.

### Создание и расположение файла .gitignore

#### Основные места размещения:
1. **Корневая директория репозитория** (рекомендуется) - `/.gitignore`
2. **Любая поддиректория** - правила применяются к этой папке и её подпапкам
3. **Глобальный файл** - `~/.gitignore_global` (применяется ко всем репозиториям)
...
### Синтаксис и правила написания шаблонов

#### Основные правила:

1. **Одна строка - одно правило**
2. **Пустые строки** игнорируются
3. **Строки, начинающиеся с `#`** - комментарии
4. **Пробелы в начале/конце** обрезаются (кроме экранирования)

#### Специальные символы:

#### 1. Звёздочка `*` - любое количество символов
```
*.log        # все файлы с расширением .log
temp*        # все файлы, начинающиеся с "temp"
```

#### 2. Вопросительный знак `?` - один любой символ
```
file?.txt    # file1.txt, fileA.txt, но не file10.txt
```

#### 3. Квадратные скобки `[]` - диапазон символов
```
[abc].txt    # a.txt, b.txt, c.txt
[0-9].log    # 0.log, 1.log, ..., 9.log
```

#### 4. Двойная звёздочка `**` - любое количество директорий
```
**/node_modules/    # node_modules в любой папке
docs/**/*.bak       # все .bak файлы в docs и подпапках
```

#### 5. Слэш `/` - указание на директории
```
/build/          # игнорировать папку build в корне
src/*.tmp        # игнорировать .tmp файлы только в src/
```
...
#### Игнорирование папок:
```
# Все эти варианты игнорируют папку
logs/
/logs/
logs/*
```

#### Исключения (инверсия) с `!`:
```
# Игнорировать все .txt файлы, кроме important.txt
*.txt
!important.txt

# Игнорировать все в temp, кроме config.json
temp/*
!temp/config.json
```
...
```
# Игнорировать файл с именем "file[1].txt"
file\[1\].txt

# Игнорировать файл с именем "*.important"
\*.important
```
...
# Настроить Git для его использования
git config --global core.excludesfile ~/.gitignore_global
```
...
```bash
# Проверить, будет ли игнорироваться файл
...
#### 1. Порядок обработки правил:
- Правила обрабатываются сверху вниз
- Более конкретные правила должны идти после общих
- Исключения (`!`) должны следовать после правил, которые они исключают

#### 2. Производительность:
- Избегайте слишком сложных шаблонов
- Используйте конкретные пути вместо общих шаблонов, когда возможно
- Большой `.gitignore` может замедлить работу Git

#### 3. Безопасность:
- Никогда не коммитьте чувствительные данные (пароли, ключи)
- Используйте `.gitignore` для конфиденциальных файлов
- Если конфиденциальные файлы уже были закоммичены, нужно изменить историю
...
### Пример:
```bash
# Добавили node_modules по ошибке
...
# Проверить все игнорируемые файлы в текущей директории
git ls-files --others --ignored --exclude-standard
...
```gitignore
# Неправильно
node_modules  # без слэша - может совпасть с файлом

# Правильно
node_modules/
```

#### 2. Порядок исключений:
```gitignore
# Неправильно - исключение не сработает
!important.log
*.log
...
```gitignore
# Игнорировать все .tmp файлы, кроме тех, что в src/
*.tmp
!src/*.tmp
```

#### Игнорирование по расширению, но не по имени:
```gitignore
# Игнорировать все .min.js, кроме specific.min.js
*.min.js
!specific.min.js
```

#### Рекурсивное игнорирование с исключениями:
```gitignore
# Игнорировать все в temp, кроме подпапки keep
temp/*
!temp/keep/
temp/keep/*
!temp/keep/*.txt
